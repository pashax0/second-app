...
|---|---|---|
| `in_stock` | Принят в фонд. У нас на руках, на витрине нет. Может быть привязан к одному или нескольким будущим (scheduled) дропам как план. | start → `listed`, → `written_off` |
...
| `complete_order(order_id)` | UPDATE products в order_items на `sold` (плюс DELETE их `drop_items` активного дропа, чтобы освободить позицию) |
...
| `list_price` | `products` | Первоначальная цена при первом листинге | задаётся раз |
| `price` | `products` | Текущая цена за которую готовы продать | да, можно снижать в фонде |
| `override_price` | `drop_items` | Цена в этом конкретном дропе (опц.) | да |
| `discount_percent` | `drops` | Drop-wide скидка (опц.) | да |
| `price_at_purchase` | `order_items` | Снапшот в момент продажи | immutable |

При публикации дропа: если `override_price` не задан, но у дропа есть `discount_percent`, то на отображении применяется `price * (1 - discount_percent/100)` (или material override — детали в [ADR при имплементации]).
...
## MVP scope (имплементация — отдельный план)

Один пакет миграций + код. Подробный план шагов — [docs/plans/products-admin.md](../../docs/plans/products-admin.md), шаг 8.

Что войдёт:

1. **Rename**: `products.status` `draft → in_stock`, `available → listed`. Добавить `written_off`.
2. **Drop status rename**: `drops.status` `draft → scheduled`.
3. **Add fields**: `products.{cost, list_price, condition, defect_notes, lot_id, deleted_at}`, `drops.discount_percent`, `orders.{cancellation_reason, cancellation_notes}`.
4. **New tables**: `supply_lots`, `returns`, `write_offs`.
5. **RPCs** (см. секцию «Консистентность.3»).
6. **Triggers** на `drop_items`, `drops`, `orders` — guard rails.
7. **RLS / column privileges**: запретить authenticated прямой UPDATE на `products.status`.
8. **View**: `products_with_flags`.
9. **Backfill**: `list_price = price` для существующих товаров; `cost = null` (заполняется по факту).
10. **Code update**:
    - admin: вызовы RPC вместо прямых UPDATE; запросы из view; UI-имена статусов; новые поля в формах.
    - mobile: типы; запросы из view там где нужны composite; никаких прямых статусных UPDATE (там их и не было).
11. **Docs sync**: [apps/admin/architecture.md](../../apps/admin/architecture.md) обновить имена.
