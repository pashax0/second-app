...
### 4. Архивные `drop_items` — иммутабельная история

**Инвариант:** строка `drop_items` дропа со `status='archived'` считается частью истории и не модифицируется.

Зачем:
- Аналитика «какой товар в каком дропе был и за сколько ушёл» опирается на эти строки.
- Один товар может побывать в нескольких дропах (вернулся со склада → попал в следующий → продан там). У такого товара несколько строк в `drop_items` — по одной на каждый дроп. Все они должны жить вечно.
- `complete_order` сохраняет `price_at_purchase` снапшотом в `order_items`, но связь «продан в каком дропе» восстанавливается через `orders.drop_id` ↔ `drop_items.drop_id`. Удалить `drop_items` archived-дропа = потерять эту привязку.

Что это значит на практике:

| Событие | Что НЕ должно происходить |
|---|---|
| `archive_drop` / авто-archive в `activate_drop` | Удалять `drop_items` архивируемого дропа. Только смена `drops.status` и пересчёт `products.status`. |
| Повторная публикация товара в новый дроп (`publish_product`) | Трогать существующие строки в archived. Создаётся новая строка в новом дропе; старая остаётся. |
| Продажа товара (`complete_order`) | Удалять любые `drop_items`. Все строки этого товара (и в active, и в archived) сохраняются. |
| Списание (`write_off_product`) | Удалять строки archived. RPC удаляет только в active-дропе. |
| Soft-delete продукта (`delete_product` для товара с историей) | Удалять `drop_items`. Скрывает товар из `products_with_flags` через `deleted_at`, но строки в `drop_items` остаются. |
| Hard-delete продукта (`delete_product` для нетронутого `in_stock`) | Это разрешено только когда у товара **нет** ни одной строки в `drop_items` — гарантирует, что hard-delete не уносит историю. |

Удаление **scheduled** или **active** дропа не запрещено инвариантом — но для archived оно недопустимо. См. также шаг 13 в `docs/plans/drops-admin.md` (delete только scheduled).

### 5. Триггеры — guard rails
...
**Известный gap:** RLS-полиси `drop_items: authenticated update/delete` (см. `20260308000002_rls_policies.sql`) сейчас разрешают любому залогиненному пользователю менять/удалять строки. UI ограничивает это правильно (через RPC для active, прямым delete только для scheduled), но БД-страховки от прямых вызовов в archived нет. Закрывается отдельной задачей вместе с переводом write-полиси на `is_admin()` (см. «Follow-up: доступ в админку» в `docs/plans/drops-admin.md`).

### 6. View `products_with_flags` — для composite-фильтров
