...
-- SECURITY DEFINER is required to update products.stock_quantity,
...
declare
  ...
  v_stock            int;
begin
  -- Verify the product belongs to this drop and get the effective price
  select
    coalesce(di.override_price, p.price),
    p.stock_quantity
  into v_price_at_purchase, v_stock
