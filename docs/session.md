...
- **Scheduled-дропы публикуются автоматически.** pg_cron `auto-activate-drops` каждые 5 минут активирует любой scheduled-дроп с `scheduled_at <= now()` через тот же путь, что и ручная кнопка (`_activate_drop_body`). Если делаешь scheduled-дроп для теста — ставь `scheduled_at` в будущее, иначе следующий тик крона его опубликует и архивирует текущий active. Решение — [ADR-005](decisions/ADR-005-auto-publish-drops.md).
